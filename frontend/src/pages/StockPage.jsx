...
import { getStock, createStock, adjustStock, transferStock } from "../api/stockService";
...
function InitStockForm({ products, stores, onDone }) {
    const [form, setForm] = useState({ productId: "", storeId: "", quantity: "" });
    ...
    const submit = async (e) => {
        ...
        if (!form.productId || !form.storeId || isNaN(qty) || qty < 0) {
    ...
    return (
        <div className="card">
            <p style={{ fontWeight: 600, marginBottom: 14, fontSize: 14 }}>Initialise Stock</p>
            <form onSubmit={submit}>
                <div className="form-row">
                    ...
                    <div className="form-field" style={{ maxWidth: 120 }}>
                        ...
                        <input type="number" name="quantity" min="0" value={form.quantity} onChange={handle} placeholder="0" />
                    </div>
...
function AdjustForm({ products, stores, onDone }) {
    ...
    const submit = async (e) => {
        ...
        if (!form.productId || !form.storeId || isNaN(delta) || delta === 0) {
            toast.error("Fill all fields; delta must be a non-zero integer");
            return;
        }
        try {
            setBusy(true);
            const res = await adjustStock({ productId: form.productId, storeId: form.storeId, delta });
            toast.success(res.message);
    ...
    return (
        <div className="card">
            <p style={{ fontWeight: 600, marginBottom: 6, fontSize: 14 }}>Adjust Stock</p>
            ...
            <form onSubmit={submit}>
                <div className="form-row">
                    ...
                    <div className="form-field" style={{ maxWidth: 120 }}>
                        <label>Delta</label>
                        <input type="number" name="delta" value={form.delta} onChange={handle} placeholder="e.g. +50 or -5" />
                    </div>
                    <button type="submit" className="btn-primary" disabled={busy} style={{ alignSelf: "flex-end" }}>
...
function TransferForm({ products, stores, onDone }) {
    const [form, setForm] = useState({ productId: "", fromStoreId: "", toStoreId: "", quantity: "" });
    const [busy, setBusy] = useState(false);
    ...
    const submit = async (e) => {
        e.preventDefault();
        const qty = parseInt(form.quantity, 10);
        if (!form.productId || !form.fromStoreId || !form.toStoreId || isNaN(qty) || qty <= 0) {
        ...
        try {
            setBusy(true);
            const res = await transferStock({ productId: form.productId, fromStoreId: form.fromStoreId, toStoreId: form.toStoreId, quantity: qty });
            toast.success(res.message);
    ...
    return (
        <div className="card">
            <p style={{ fontWeight: 600, marginBottom: 6, fontSize: 14 }}>Transfer Stock</p>
            <p style={{ fontSize: 12, color: "#888", marginBottom: 14 }}>
                Moves stock atomically — source decrements and destination increments in a single transaction.
            </p>
            <form onSubmit={submit}>
                <div className="form-row">
                    <div className="form-field">
                    ...
                    <div className="form-field">
                        ...
                        <select name="fromStoreId" value={form.fromStoreId} onChange={handle}>
                    ...
                    <div className="form-field">
                        ...
                        <select name="toStoreId" value={form.toStoreId} onChange={handle}>
                            ...
                            {stores.filter((s) => s._id !== form.fromStoreId).map((s) => (
                                <option key={s._id} value={s._id}>{s.name}</option>
                            ))}
                        </select>
                    </div>
                    <div className="form-field" style={{ maxWidth: 120 }}>
                        <label>Quantity</label>
                        <input type="number" name="quantity" min="1" value={form.quantity} onChange={handle} placeholder="e.g. 10" />
                    </div>
                    <button type="submit" className="btn-primary" disabled={busy} style={{ alignSelf: "flex-end" }}>
...
function StockPage() {
    ...
    const fetchStock = useCallback(async () => {
        try {
            setLoading(true);
            const res = await getStock(threshold !== "" ? Number(threshold) : undefined);
            setStocks(res.data || []);
        } catch (err) {
    ...
    return (
        <div className="page-body">
                                ...
                                <tbody>
                                    {stocks.map((s) => (
                                        <tr key={s._id}>
                                            ...
                                            <td><span className="sku">{s.product?.sku ?? "—"}</span></td>
                                            <td>{s.store?.name ?? "—"}</td>
            ...
            {activeTab === "Initialise" && isAdmin && (
                <InitStockForm products={products} stores={stores} onDone={fetchStock} />
            )}
        </div>
    );
}
